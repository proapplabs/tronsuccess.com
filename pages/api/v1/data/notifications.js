...
import { getUserTokenDB, getDomainTokenDB, getDomainPlansDB, getNotificationsDB } from '@utils/v1/supabase'
    ...
    if (account) {
        ...
        if (DOMAIN_TOKEN_DECODE.name == USER_TOKEN_DECODE.sub) {
            ...
            if (user && domain) {
                ...
                const items = await getDomainPlansDB(domain_id, true)
                let userNotifications = await getNotificationsDB(user_id, false)

                userNotifications.map((n, i) => {
                    n.content = n.content.replace("(invitation code:)", "(invitation code:" + inviteCode + ")")
                    return n
                })
