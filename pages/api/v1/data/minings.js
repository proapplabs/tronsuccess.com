...
import { percent } from '@utils/v1/func'
import { apiKeyControl, emptyBody, getApiKey } from '@utils/v1/api'
import { getUserTokenDB, getDomainTokenDB, getUserPlansDB, updateIdDB } from '@utils/v1/supabase'
    ...
    if (account) {
        ...
        if (DOMAIN_TOKEN_DECODE.name == USER_TOKEN_DECODE.sub) {
            ...
            if (user && domain) {
                ...
                const basic_total = percent(user.basic_total, domainRate)
                const domain_id = domain.id
                const items = await getUserPlansDB(domain_id, user_id)
